...
# maps = np.zeros((ROWS, ROWS))
maps = create_random_map(100, 100, 0.03, 2, 30, seed=413)
...
class SnakeGame(object):
    ...
    def adjmatrix_noreturn(self, N, explored_grid):
        ...
        for i in range(len(N)):
            for j in range(i, len(N)):
                if i == j:
                    ...
                    start = explored_grid[N[i][0]][N[i][1]]
                    end = explored_grid[N[j][0]][N[j][1]]
                    flag, path = self.A_star(explored_grid, start, end)
                    t = 0
                    ...
                    for q in list(range(0, len(path) - 1)):
                        curr_node = path[q]
                        next_node = path[q + 1]
                        for pos, neighbor in curr_node.neighbors.items():
                            if neighbor == next_node:
                                move_position = pos
                                break
                        if move_position == 'UP' or move_position == 'DOWN' or move_position == 'LEFT' or (
                                move_position == 'RIGHT'):
                            t += 1
                        else:
                            t += 1.4
    ...
    def get_all_vics(self, snake_map_pos, explored_grid):
        victims = []
        for i in range(0, ROWS):
            for j in range(0, ROWS):
                if explored_grid[i][j].is_end() and not explored_grid[i][j].surround_by_barriers():
                    start = explored_grid[snake_map_pos[0]][snake_map_pos[1]]
                    end = explored_grid[i][j]
                    flag, path = self.A_star(explored_grid, start, end)
                    if path:
                        victims.append([i, j])
        return victims  # a list of victims indices in grid
    ...
    def play_game(self, map_reference, explored_map, true_map):
        ...
        while explore or tsp:
            ...
            # the 2nd strategy
            # traverse the patrol route
            if explore:
                if (not explored_grid[explore_position[0]][explore_position[1]].is_barrier()) and (not been_there):
                    ...
                    # when there is a barrier in an explore site, check unexplored parts around the site
                    if not been_there:
                        been_there = True
                        ...
                        if (snake_map_pos == unexplored_part) or (
                            ...
                            f, move_one_block = self.explore(snake_map_pos, unexplored_part, explored_grid,
                                                             explored_map)
                            if f:
                                snake_position = self.move_one_step(snake_position, move_one_block, explored_grid)
                            else:
            ...
            if tsp:
                if not tsp_path:
                    explore_step = self.step
                    dests = self.get_all_vics(snake_map_pos, explored_grid)  # coordinates of survivors in grid
                    dests.insert(0, snake_map_pos)
                    # tsp_map = Generate_Map.make_block_map(explored_map)
                    Adj = self.adjmatrix_noreturn(dests, explored_grid)
                    [cost, tsp_path] = Salesman_noreturn.TSP_ACO(Adj)
                    curr_pos = 0
                    been_there = False
                else:
                    tsp_idx = tsp_path[curr_pos]
                    tsp_pos = dests[tsp_idx]
                    if not been_there:
                        if snake_map_pos != tsp_pos:
                            f, move_one_block = self.explore(snake_map_pos, tsp_pos, explored_grid,
                                                             explored_map)
                            if f:
                                snake_position = self.move_one_step(snake_position, move_one_block, explored_grid)
                            else:
        ...
        end_time = time.time()
        # print('computed cost:', explore_step + cost)
...
if __name__ == "__main__":
    ...
    map_indices = [413, 389, 204, 613, 183, 235, 254, 136, 778, 88, 257, 746, 392, 543, 700, 717, 551, 91, 960, 638, 501,
               431, 739, 627, 602, 221, 811, 923, 807, 382, 842, 397, 882, 698, 217, 155, 905, 676, 870, 580, 346, 84,
               969, 733, 541, 693, 793, 89, 480, 323]
    # print(map_indices)
    ...
    with open(r"2st_Strategy_Summary.txt", 'w') as f:
        f.write("Mean: ")
