...
maps = create_random_map(100, 100, 0.03, 2, 30, seed=580)
...
class SnakeGame(object):
    ...
    def explore(self, snake_map_position, explore_position, explored_grid, explored_map):
        ...
        flag = False
        move_one_block = None
        input_map = Generate_Map.make_block_map(explored_map)
        path = JPS.find_path(snake_map_position, explore_position, ROWS, ROWS, input_map)
        if path:
            flag = True
            move_one_block = path[1]  # next pos in row col format
    ...
    def move_one_step(self, snake_position, end, explored_grid):
        ...
        end_row = end[0]
        end_col = end[1]
        change_row = start_row - end_row
        change_col = start_col - end_col
        # Moving the snake
        if change_row > 0 and change_col == 0:  # UP
            snake_position[1] -= self.block_size
        elif change_row < 0 and change_col == 0:  # DOWN
            snake_position[1] += self.block_size
        elif change_row == 0 and change_col > 0:  # LEFT
            snake_position[0] -= self.block_size
        elif change_row == 0 and change_col < 0:  # RIGHT
            snake_position[0] += self.block_size
        elif change_row > 0 and change_col > 0:  # UP_LEFT
            snake_position[1] -= self.block_size
            snake_position[0] -= self.block_size
        elif change_row > 0 and change_col < 0:  # UP_RIGHT
            snake_position[1] -= self.block_size
            snake_position[0] += self.block_size
        elif change_row < 0 and change_col > 0:  # DOWN_LEFT
            snake_position[1] += self.block_size
            snake_position[0] -= self.block_size
        elif change_row < 0 and change_col < 0:  # DOWN_RIGHT
            snake_position[1] += self.block_size
            snake_position[0] += self.block_size
